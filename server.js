/**
 * PepetaHigh — Backend (UPDATED WITH DATABASE + AUTH + WALLET)
 */
...
const db = new sqlite3.Database('./pepetahigh.db', (err) => {
...
app.post('/api/auth/register', (req, res) => {
  ...
  db.run(
    `INSERT INTO users (username, phone, password, wallet, createdAt)
     VALUES (?, ?, ?, ?, ?)`,
    [username, phone, hashed, 0, Date.now()],
    function (err) {
...
app.get('/api/wallet/:username', (req, res) => {
  ...
  db.get(`SELECT wallet FROM users WHERE username = ?`, [username], (err, user) => {
    if (!user) return res.status(404).json({ error: 'User not found' });
